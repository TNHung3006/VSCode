...
int main() {	// cách xuất mảng 2 chiều
	...
	int c[][COL] = { 	// cách 3 // ko thể để trống cả 2 chỉ đc để 1
		{1, 2, 3}, 
		{7, 8}, 
		{9} };

	int d[i][j] {}; // khởi tạo tất cả giá trị trong mảng bằng 0
...
int main(){ // nhập hàng và cột của mảng 2 chiều, phần tử random
	...
	int a[ROW][COL];	// số dòng và số cột lớn nhất mà mảng có thể nhận

	srand(time(NULL));
	int row, col;	// số dòng và số cột tuỳ theo í mình trong giới hạn của mình đã đặt trước
	cout << "nhap so hang: ";
	cin >> row;
	cout << "nhap so cot: ";
	cin >> col;
