...
void themMotPhanTuVaoMang(int a[], int& n, int idx, int x)
{
	if (idx >= 0 && idx <= n) // idx =3
	{
		for (int i = n; i > idx; i--) // i = 5, 4; i > 3 ; i--
			a[i] = a[i - 1]; //a[5] = a[4], a[4] = a[3]
		a[idx] = x; // a[3] = 7310
		n++; // n = 6
	}else
	cout << "vi tri khong co trong mang";
}
...
void xoaMotPhanTuTrongMang(int a[], int& n, int idx)
{
	if (idx >= 0 && idx < n) //idx = 3
	{
		for (int i = idx; i < n - 1; i++) // i = 3 4; i < 6 - 1, i++
			a[i] = a[i + 1]; //a[3] = a[4]; a[4] = a[5]
		n--; // n = 5
	}
}
